...
class AiGovernanceProposalPreconditionValidator {
    fun validate(
        ...
        if (!ControlledIdentifier.isLocal(request.batchId)) errors += "batchId must use the controlled local identifier vocabulary"
        ...
        if (!ControlledIdentifier.isLocal(request.modelId)) errors += "modelId must use the controlled local identifier vocabulary"
        if (!ControlledIdentifier.isLocal(request.promptId)) errors += "promptId must use the controlled local identifier vocabulary"
...
class AiGovernanceProposalValidationGate(
    ...
    private fun validateShacl(model: Model): List<String> {
        return validationEngine.validate(
            model,
            SemanticValidationProfile.AI_GOVERNANCE,
            "AI governance SHACL validation failed",
        )
    }
